...
qualifies_double_grade = pd.read_csv("8_class/data/double_grade.csv")
...
number_of_folds = 4
cv_model_quality = sk_model_selection.cross_val_score(
    cv_qualification_model, X, y, cv=number_of_folds, scoring="precision"
)
print(cv_model_quality)

cv_prediction = sk_model_selection.cross_val_predict(
    cv_qualification_model, X, y, cv=number_of_folds
)
...
modeled_probability = qualification_model.predict_proba(X)[:, 1]

qualifies_double_grade["modeled_probability"] = modeled_probability
...
print(qualification_model0.intercept_)
